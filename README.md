...
这是一个直接运行于 Node.js 的股票监控服务，提供自选股行情、全球市场概览、K 线、异动消息以及新股和可转债申购日历，并在现有服务中集成微信公众号回调、菜单以及每周汇总和当日打新提醒。
...
- “投资工作台”提供浏览器本地的模拟持仓与买卖记录、实时估值、集中度风险、自定义价格预警、事件日历、F10 研究卡入口、AI 摘要入口、日 K 行情回放、5/20 日均线简化回测和 CSV 导出；不连接真实券商，也不会发出交易指令。
...
- 账号首次登录会询问是否关联当前页面配置。选择“关联当前配置”后，股票与基金行情分组、主题、指标、行情颜色、刷新频率、当前页面和异动记录会保存到该账号。
...
- 可搜索并添加沪深 ETF；支持实时行情、分时、分时 K 和日 K。ETF 会以独立标签显示，且不展示公司估值、财务、所属板块和个股主力资金等不适用指标。
- ETF 主力资金需要独立的数据口径，当前不与普通 A 股东方财富主力资金曲线混用。

### 实时行情降级与状态

- `/api/quote` 单次最多接受 100 个受支持的沪深、港股或美股代码，并对上游响应设置 8 秒超时和 2MB 大小上限。
- 服务端按股票保存最近 15 分钟的成功行情快照。腾讯行情暂时失败或只返回部分股票时，会按股票补齐仍有效的最近快照，并通过响应头标明实时、部分数据或缓存降级状态。
- 浏览器保留已经成功展示的行情，但会把本轮缺失的股票标为“延迟”；完全失败时继续展示现有表格并提示刷新失败，不会用空白表格覆盖最近可用数据。

- 普通 A 股“主力资金”页只请求东方财富 `daykline`，统一展示其最近 120 个交易日的主力净流入、超大单净额和大单净额。新浪对大单/主力的分类口径不同，可能与东方财富出现相反方向，因此不再作为该指标的备用源。
...
- 沪深 A 股详情提供“资讯财报”页，聚合最近报告期的营业收入、归母净利润、ROE、毛利率、每股收益及同比变化，并展示最近 12 个法定报告期的累计口径趋势表。
- 财报原文和公司公告来自东方财富公开公告页面，页面只保存标题、分类、披露时间和公开 PDF 链接；相关新闻来自新浪财经公开个股资讯页，只展示标题、来源、时间和原文链接，不复制新闻正文。
- 服务端分别对公告、财务和新闻设置有界缓存、请求超时、响应大小限制及每 IP 限流。三个来源独立加载，单个来源失败时页面保留其余可用内容，并在已有缓存可用时明确标注缓存状态。
- 公开网页接口可能延迟、限流或调整，新闻关联也可能存在遗漏。页面内容仅供信息参考，不构成投资建议；公告和财务数据请以巨潮资讯、交易所及上市公司正式披露原文为准。

### 资金流向中心

- 侧边栏“资金流向”集中展示沪市、深市及沪深合计的当日分钟资金曲线，并拆分主力、超大单、大单和中小单净额。大盘历史暂不可用时会保留可用的分钟数据，反之亦然。
- 板块榜覆盖行业、概念和地域，个股榜覆盖沪深 A 股、主板、创业板、科创板及沪深市场筛选；两类榜单均支持今日、3 日、5 日、10 日和净流入/净流出方向切换。
- 点击榜单项目可查看最近 120 个交易日和当日分钟资金，历史页会计算最新连续净流入或净流出天数与期间累计金额。板块详情也可直接跳到资金历史，资金页可反向打开板块详情或个股行情。
- 自选异动扫描在原有涨跌、快速拉升/跳水和涨跌停监控之外，会识别主力大额净流入/流出及有一定金额基础的资金方向反转。异动仍仅保存在当前浏览器。
- 浏览器只访问本站 `/api/capital-flow/*` 代理。服务端对公开资金接口限流，并使用有界缓存、请求队列、`push2` 主源和 `push2delay` 延迟镜像；实时榜单和分钟数据采用秒级短缓存，历史数据采用 15 分钟缓存。
- “主力”来自成交单规模分类，并非已确认的机构账户身份。上游属于网页行情接口、没有稳定性承诺，页面会标示缓存、部分数据或暂不可用状态。

### 期指与美股夜盘

- 侧边栏“期指与夜盘”展示 IF（沪深300）、IH（上证50）、IC（中证500）、IM（中证1000）连续合约，包含最新价、相对昨结涨跌、成交量和持仓量；同时读取对应现货指数，计算“期货价 − 现货指数”的实时基差及基差率。
- 美股夜盘区展示 ES（E-mini 标普500）、NQ（E-mini 纳斯达克100）和 YM（E-mini 道琼斯）指数期货。页面把美股现货的盘前、常规、盘后时段与 CME 指数期货时段分开标识，避免把指数期货的近全天交易误认为美股现货成交。
- 资讯区聚合东方财富期货快讯；全球股市快讯会在服务端优先筛选美股、三大指数、美联储和重点科技股相关内容。资讯链接仅允许东方财富 HTTPS 域名，并统一在新标签页打开。
- 浏览器仅访问本站 `/api/derivatives/overview`。服务端代理新浪财经公开行情及东方财富公开快讯，行情缓存 5 秒、资讯缓存 2 分钟，并带有每 IP 限流、请求超时、响应大小检查和部分数据降级。
- 页面附有中金所、CME 与 Nasdaq 官方资料入口。行情和资讯可能延迟；连续合约、基差和指数期货方向仅供市场研究，不构成投资建议。
...
- 每周一 `09:00`（`Asia/Shanghai`）汇总并发送本周可申购新股和新债。
...
每日任务仅在当天存在可申购新股或新债时创建并发送；当天没有打新标的时不会发送空提醒。
...
当天无可申购新股或新债时返回 `accepted=false`，不会创建任务；当天任务已经完成时不会重复发送。失败用户可在确认故障排除后增加 `?retry_failed=1` 重试。
...
时区：Asia/Shanghai
每周汇总：星期 1（周一）09:00
当日提醒：每天 09:00，且仅在当天有可申购新股或新债时发送
补执行：开启
```
...
新债 1 只：
1. 示例转债（371002）【可转债】
   2026-07-30 周四｜评级 AA-｜发行规模 5.50亿元
...
当日提醒格式为 `【今日打新申购提醒】`，只列出 `applyDate` 等于当天的新股和新债；其余字段和长度限制与周报一致。
