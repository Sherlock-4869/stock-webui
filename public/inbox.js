(function () {
  ...
  async function deleteAlert(id) { if (!window.currentUser || !window.confirm('确定删除这条价格预警消息吗？')) return; try { await request(`/api/inbox/${encodeURIComponent(id)}`, { method:'DELETE', body:'{}' }); await refresh(); } catch (error) { setStatus(error.message || '删除失败', true); } }
  ...
  window.Inbox = { refresh, markAllRead, deleteAlert, pushAlert, adminRefresh, adminSend, adminDelete };
